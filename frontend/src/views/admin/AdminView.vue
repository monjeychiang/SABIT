  ...
  <div class="admin-page">
    ...
    <div class="dashboard-stats">
      ...
      <div class="stat-card">
        ...
        <div class="stat-content">
          ...
          <p class="stat-description" v-if="userCount > 0">活跃率: {{ Math.round((onlineUserCount / userCount) * 100) }}%</p>
        </div>
      </div>
    ...
    <div class="admin-container">
      <div class="admin-section">
        <h2>在线用户</h2>
        <OnlineUsersWidget :refresh-interval="30000" :max-users="10" />
      </div>
    </div>
    ...
    <div class="admin-container">
      <div class="admin-section">
        ...
        <div class="error-message" v-else>
          <p>无法获取服务器状态信息</p>
          <button class="btn btn-danger" @click="fetchServerStatus">重试</button>
        </div>
      </div>
    </div>
            ...
            <div class="form-group admin-password-section">
              ...
              <div class="password-field">
                ...
                <button type="button" class="toggle-button" @click="toggleAdminPasswordVisibility">
                  <span>{{ showAdminPassword ? '隐藏' : '显示' }}</span>
                </button>
              </div>
          ...
          <form @submit.prevent="sendNotification">
            ...
            <div class="form-group">
              <div class="checkbox-wrapper">
                ...
                >
                <label for="is-global">发送给所有用户</label>
              </div>
            </div>
            
            <div class="form-group" v-if="!notificationForm.is_global">
              <label for="selected-users">选择接收者</label>
            ...
            <div class="modal-footer">
              ...
              >
                {{ isSendingNotification ? '发送中...' : '发送通知' }}
              </button>
            </div>
          </form>
        </div>
...
const perPage = ref(10);  // 确保每页显示10条记录
...
// 确保通知样式已正确加载
console.log('通知样式：', notificationStyles.value);
...
const selectNotificationType = (type) => {
  ...
  console.log('选择通知类型:', notificationForm.value.notification_type);
};

const selectableUsers = ref([]); // 可选择的用户列表
const selectedUsers = ref([]); // 已选择的用户
const userSearchQuery = ref(''); // 用户搜索关键字
const showUserSelector = ref(false); // 是否显示用户选择器
...
const fetchUserLoginHistory = async () => {
  // 此函数已被废弃，用户登录历史现在通过用户详情页面查看
  console.log('此函数已被废弃，用户登录历史现在通过用户详情页面查看');
};
...
onMounted(() => {
  ...
  onlineUserRefreshInterval.value = setInterval(fetchOnlineUserCount, 30000); // 每30秒刷新
...
const updateUser = async () => {
  ...
  try {
    ...
    // 如果用户原本是管理员，但尝试移除管理员权限，显示警告并保持管理员权限
    if (wasAdmin && !isAdmin) {
    ...
    await axios.put(`/api/v1/admin/users/${editingUser.value.id}`, {
      ...
      is_active: isActive,
      is_admin: isAdmin,
      admin_password: adminPassword.value  // 传递管理员密码用于后端二次验证
    }, {
      headers: {
        'Authorization': `Bearer ${authStore.token}`
      }
...
const openSendNotificationModal = () => {
  console.log('Opening notification modal'); // 添加调试日志
  notificationForm.value = {
    title: '',
    message: '',
    is_global: false,
    user_ids: [],
    notification_type: 'info' // 确保使用小写字符串
  };
  selectedUsers.value = [];
...
const viewUserDetails = async (user) => {
  ...
  try {
    ...
    console.log('用户详细资料:', userDetails.value);
  } catch (error) {
    console.error('获取用户详细资料失败:', error);
    showToast('获取用户详细资料失败', 'error');
    ...
    try {
      ...
      console.error('获取基本用户信息也失败:', fallbackError);
    }
  } finally {
...
// 在script部分添加新的变量和计算属性
// 在现有变量声明后添加
const jumpPage = ref(1);

// 在computed部分添加显示页码的计算属性
